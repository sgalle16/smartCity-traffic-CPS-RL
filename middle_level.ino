...
void webSocketEvent(WStype_t type, uint8_t * payload, size_t length) {
    switch(type) {
        ...
        case WStype_TEXT: {
            ...
            const char* msg = doc["msg"]; // Extraer el comando
...
void setup() {
  ...
  lcd.backlight(); // Turn on LCD backlight
   ...
   // --- NUEVA SECCIÓN: CONEXIÓN A LA RED ---
  lcd.clear();
...
void loop() {
  ...
  displayInfo();
    ...
    // --- NUEVA SECCIÓN: ENVÍO DE DATOS POR WEBSOCKET ---
...
void controlarSemaforos() {
  ...
  if (modoOperacion == MODO_DIA) { // --- Logica de la MEF para el MODO DÍA ---
    ...
    long duracionAmarillo = 500;       // 1.5 segundos
    long duracionPasoPeatonal = 3000;  // 4 segundos
    // Dinamic Tiempo Verde (acelerado)
    long duracionVerdeBase = 2000;      // 3 segundos base
