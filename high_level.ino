...
// --- CONFIGURACIÓN DE WIFI Y WEBSOCKET ---
const char *ssid = "iPhoneMW";
const char *password = "123456789";
const char *websocket_server_host = "ws.davinsony.com";
const char *websocket_server_path = "/city_2";
const uint16_t websocket_server_port = 443;

// --- IDs y Canales ---
const char* ACTOR_ID = "actor_semaforo_1"; // Nombre único de nuestro actor
const char* CANAL_WEBSOCKET = "city_2";   // Canal al que nos conectamos y enviamos
const char* BRAIN_ID = "Brain_RL";
...
bool modoLluvia = false; // El supervisor de Nivel Alto controlará esto

// Library definitions
LiquidCrystal_I2C lcd(0x27, 20, 4);
WebSocketsClient webSocket;
unsigned long ultimaVezEnviado = 0;
...
#define UMBRAL_LUZ_NOCHE 400     // Valor bajo para LDR indica oscuridad
...
void webSocketEvent(WStype_t type, uint8_t * payload, size_t length) {
    switch(type) {
        ...
        case WStype_CONNECTED:
            ...
            break;
        case WStype_TEXT: {
            // --> CORRECCIÓN 1: Declarar y parsear el JSON aquí <--
            StaticJsonDocument<256> doc;
            deserializeJson(doc, payload);
            const char* msgType = doc["type"]; // Extraer el comando

            // Si el mensaje no tiene el campo "msg", no hacer nada
            if (!msgType) return;

            // --- LÓGICA PARA COMANDOS DEL CEREBRO TÁCTICO (NIVEL MEDIO) ---
            if (strcmp(msgType, "BRAIN_COMMAND") == 0) {
            const char* msg = doc["msg"];
            if (!msg) return;
            Serial.printf("[WSc] Brain RL command: %s\n", msg);
            
            if (strcmp(msg, "ATENDER_PEATON") == 0) {
                // El cerebro da la orden, el actor decide a quién atender
                if (peticionPeatonS1) {
                  estadoMEF = PASO_PEATONAL_S1;
                  peticionPeatonS1 = false; // Resetea la petición
                  tini = millis();
                  Serial.println(">>> Peticion S1 ATENDIDA. Iniciando paso peatonal.");
              } else if (peticionPeatonS2) {
                  estadoMEF = PASO_PEATONAL_S2;
                  peticionPeatonS2 = false; // Resetea la petición
                  tini = millis();
                  Serial.println(">>> Peticion S2 ATENDIDA. Iniciando paso peatonal.");
              }
          } 
            else if (strcmp(msg, "CAMBIAR_A_ROJO") == 0) {
                if(estadoMEF == VERDE_S1_ROJO_S2) estadoMEF = AMARILLO_S1_ROJO_S2;
                if(estadoMEF == ROJO_S1_VERDE_S2) estadoMEF = ROJO_S1_AMARILLO_S2;
                tini = millis();
            }
            else if (strcmp(msg, "FORZAR_CAMBIO") == 0) {
              if(estadoMEF == VERDE_S1_ROJO_S2) estadoMEF = AMARILLO_S1_ROJO_S2;
              else if(estadoMEF == ROJO_S1_VERDE_S2) estadoMEF = ROJO_S1_AMARILLO_S2;
              tini = millis();
              Serial.println("[WSc] FORZAR_CAMBIO de luces");
            }          
            else if (strcmp(msg, "SEGUIR_CICLO") == 0) {
              Serial.println("[WSc] SEGUIR_CICLO de luces");
            }
          } // --- LÓGICA PARA COMANDOS DEL SUPERVISOR (NIVEL ALTO) ---
            else if (strcmp(msgType, "STRATEGIC_COMMAND") == 0) {
                const char* comando = doc["comando"];
                Serial.printf("[WSc] Comando Estratégico Recibido: %s\n", comando);
    
                if (strcmp(comando, "MODO_LLUVIA_ON") == 0) {
                    modoLluvia = true;
                } else if (strcmp(comando, "MODO_LLUVIA_OFF") == 0) {
                    modoLluvia = false;
                }
            }
          break;
...
void setup() {
  ...
  Serial.println("Smart City - Sistema Control Trafico - Nivel Medio - INICIADO");
  delay(1500);
...
void loop() {
  ...
  if (millis() - ultimaVezEnviado > 5000 && webSocket.isConnected()) {
    ultimaVezEnviado = millis();
    
    StaticJsonDocument<512> estadoJSON;
    estadoJSON["from"] = ACTOR_ID;
    estadoJSON["to"] = CANAL_WEBSOCKET;
    estadoJSON["msg"] = "Estado del Actor";
    // Añadimos el campo "type" que el cerebro espera para identificar el mensaje.
    estadoJSON["type"] = "ACTOR_STATE";
    // Empaquetar todos los datos de los sensores
    estadoJSON["estadoMEF"] = estadoMEF;
    estadoJSON["modoOperacion"] = (modoOperacion == MODO_DIA) ? "DIA" : "NOCHE";
    estadoJSON["peticionPeatonS1"] = peticionPeatonS1;
    estadoJSON["peticionPeatonS2"] = peticionPeatonS2;
    estadoJSON["LDR1_val"] = semaforos[0].vLDR;
    estadoJSON["LDR2_val"] = semaforos[1].vLDR;
    estadoJSON["traficoS1_peso"] = semaforos[0].pesoTrafico;
    estadoJSON["traficoS2_peso"] = semaforos[1].pesoTrafico;
    estadoJSON["CO2_ppm"] = dCO2;
    
    String jsonEstado;
    serializeJson(estadoJSON, jsonEstado);
    
    Serial.print("Enviando estado: ");
    Serial.println(jsonEstado);
    
    webSocket.sendTXT(jsonEstado);
  }
}
...
void controlarSemaforos() {
  ...
  if (modoOperacion == MODO_DIA) { // --- Logica de la MEF para el MODO DÍA ---
    ...
    // Tiempos fijos (acelerados)
    long duracionAmarillo = modoLluvia ? 3000 : 1500; // 3s si llueve, 1.5s si no
    long duracionPasoPeatonal = modoLluvia ? 8000 : 5000; // 8s / 5s
    // Dinamic Tiempo Verde (acelerado)
  ...
  if (tactual - lastDisplayTime > 500) {
    ...
    if (modoOperacion == MODO_DIA || estadoMEF == PASO_PEATONAL_S1 || estadoMEF == PASO_PEATONAL_S2) {
        ...
        switch(estadoMEF) {
            ...
            case PASO_PEATONAL_S1:
            case PASO_PEATONAL_S2:
                duracionTotal = 3000; countdown = (duracionTotal - tdelta)/1000 + 1;
